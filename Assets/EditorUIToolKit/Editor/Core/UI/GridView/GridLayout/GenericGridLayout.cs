...
public class GenericGridLayout : GridLayout
{
    ...
    protected void DrawSubAssetRowBg(int startSubAssetIndex, int endSubAssetIndex, bool continued)
    {
        ...
        float num4 = 9f * num3;
        float num5 = 4f;
        float num6 = startSubAssetIndex % this.LayoutParams.Columns != 0 ? this.LayoutParams.HorizontalSpacing + num3 * 10f : 18f * num3;
        ...
        (!continued ? GridView.s_Styles.subAssetBgCloseEnded : GridView.s_Styles.subAssetBgOpenEnded).Draw(position2, GUIContent.none, false, false, false, false);
    ...
    {   
        ...
        while (true)
        {
            ...
            DrawItem(LayoutParams.CalculateItemRect(itemIndex), m_DataSource.GetItemIdByIndex(itemIndex), itemIndex);
            ++itemIndex;
        }
    }
    ...
    private void DrawItem(Rect itemRect, int itemId, int itemIndex)
    {
        ...
        bool isSelected = !m_Owner.ViewConfig.AllowDragging ? m_Owner.IsSelected(itemId) : (m_DragSelection.Count <= 0 ? m_Owner.IsSelected(itemId) : m_DragSelection.Contains(itemId));
        ...
        if (hasChild && !ListMode)
        {
            ...
            expandBtnRect = new Rect(itemRect.xMax - width * 0.5f, itemRect.y + ((itemRect.height - GridView.s_Styles.resultsGridLabel.fixedHeight) * 0.5f - width * 0.5f), width, height);
        }

        //快捷键展开Item的子资源的事件处理
        bool changeState = false;
        if (isSelected && !m_Owner.ViewHandler.SearchMode && current.type == EventType.KeyDown && m_Owner.HasFocus())
        {
        ...
        //点击到了网格模式下Item的展开按钮
        if (hasChild && !item.IsChildItem && current.type == EventType.MouseDown && (current.button == 0 && expandBtnRect.Contains(current.mousePosition)))
            changeState = true;
        ...
        if (!ListMode)
            displayNameRect = new Rect(itemRect.x, itemRect.yMax + 1f - GridView.s_Styles.resultsGridLabel.fixedHeight, itemRect.width - 1f, GridView.s_Styles.resultsGridLabel.fixedHeight);
        ...
        if (Event.current.type == EventType.Repaint)
        {
            ...
            bool isDraging = controlId == m_DropTargetControlID && m_DragSelection.IndexOf(m_DropTargetControlID) == -1;
            if (ListMode)        //列表模式的item绘制
            {
                ...
                {
                    //绘制选中背景
                    GridView.s_Styles.resultsLabel.Draw(itemRect, GUIContent.none, false, false, true, m_Owner.HasFocus());
                }
                ...
                //绘制图标和文本
                DrawIconAndLabel(new Rect(x, itemRect.y, itemRect.width - x, itemRect.height), itemName, m_Content.image as Texture2D, isSelected, m_Owner.HasFocus());

                //在列表模式中，如果一个对象有子对象可以有一个箭头按钮指示
                if (hasChild && !item.IsChildItem)
                {
                    //绘制最左边的展开和收缩小箭头
                    GridView.s_Styles.groupFoldout.Draw(expandBtnRect, !ListMode, !ListMode, IsExpanded(itemId), false);
                }  
            }
            else
            {
                ...
                Rect rect = m_Content.image != null ? GetImageDrawPosition(itemRect, m_Content.image.width, m_Content.image.height) : new Rect();
                m_Content.text = null;
                ...
                {
                    GridView.s_Styles.resultsGrid.Draw(rect, m_Content, false, false, isSelected, m_Owner.HasFocus());
                }
                ...
                if (!isRenaming)
                {
                    ...
                    {
                        //绘制外圈发光
                        GridView.s_Styles.resultsLabel.Draw(new Rect(displayNameRect.x - 10f, displayNameRect.y, displayNameRect.width + 20f, displayNameRect.height), GUIContent.none, true, true, false, false);
                    }

                    string croppedLabelText = m_Owner.GetCroppedLabelText(itemId, itemName, itemRect.width);
                    GridView.s_Styles.resultsGridLabel.Draw(displayNameRect, croppedLabelText, false, false, isSelected, m_Owner.HasFocus());
                }

                if (hasChild && !item.IsChildItem)
                {
                    //绘制展开按钮
                    GridView.s_Styles.subAssetExpandButton.Draw(expandBtnRect, !ListMode, !ListMode, IsExpanded(itemId) && !item.IsChildItem, false);
                }    
            }
        }
    ...
    protected override void HandleUnusedDragEvents(float yOffset)
    {
        ...
        switch (current.type)
        {
            ...
            case EventType.DragPerform:
                if (!new Rect(m_Owner.m_TotalRect.x, m_Owner.m_TotalRect.y, m_Owner.m_TotalRect.width, m_Owner.m_TotalRect.height <= LayoutParams.Height ? LayoutParams.Height : m_Owner.m_TotalRect.height).Contains(current.mousePosition))
                    break;

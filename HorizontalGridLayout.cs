...
/// </summary>
public class HorizontalGridLayout : LayoutGroup, IDynamicLayout
{
    [Serializable]
    ...
    [SerializeField] protected ScrollView m_ScrollView; //水平滑动的ScrollView, 布局监听ScrollView的onContentPosChanged来动态布局

    [SerializeField] protected bool m_ExpandHeight; //强制Item的高度自适应可视区域的高,水平滑动的时候有效
    ...
    [SerializeField] protected bool m_EnableArrowHintEvent; //是否开启箭头指示事件, 可选用指示当前viewport向左或向右拖拽是否有内容可拖拽
    ...
    public bool expandHeight
    {
        ...
        set { SetProperty(ref m_ExpandHeight, value); }
    }
    ...
    protected void CalculateWidth()
    {
        ...
        var totalColumn = itemCount / row;
        totalColumn += itemCount % row > 0 ? 1 : 0;

        var contentWidth = totalColumn * itemWidth + (totalColumn - 1) * spacing.y + padding.horizontal;
    ...
    protected void GetVisibleItemRange(ref int firstItemIndex, ref int itemCount)
    {
        ...
        var deltaWidth = -contentTopLeft.x - padding.left;
        var itemWidth = m_Adapter.GetItemSize().x + spacing.x;
        var curColumnNumber = Mathf.FloorToInt(deltaWidth / itemWidth);

        firstItemIndex = curColumnNumber * row;
        var viewportWidth = m_ScrollView.viewport.rect.width;
        var deltaWidthToRight = viewportWidth - contentTopLeft.y - padding.top;
        if (firstItemIndex < 0)
        {
            deltaWidthToRight = viewportWidth - padding.top;
            firstItemIndex = 0;
            curColumnNumber = 0;
        }

        var showWidth = deltaWidthToRight - itemWidth * curColumnNumber;
        var showTotalColumn = Mathf.CeilToInt(showWidth / itemWidth);
        itemCount = showTotalColumn * row;
